...
export const fishCodegen: ShellCodegen = {
  ...
  sourceBlock(scriptPath) {
    ...
    return ['# >>> claudefob >>>', `test -f ${p}; and source ${p}`, '# <<< claudefob <<<'].join('\n')
  },
  hookBlock() {
    return [
      '# >>> claudefob >>>',
      ...
      '# `string collect` is required: fish splits command substitution on newlines, so a',
      '# multi-line export would be flattened into a single command and the token would swallow',
      '# the line after it.',
      'eval (command claudefob export --shell fish | string collect)',
      ...
      '    end',
      '    touch "$__cf_marker" 2>/dev/null',
      '    set -l __cf_out (command claudefob export --shell fish --sync 2>/dev/null | string collect)',
      '    or return 0',
      '    test -n "$__cf_out"; and eval $__cf_out',
      '    return 0',
      'end',
